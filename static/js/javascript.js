...
function initializeChart(chartId, chartData) {
  ...
  var lineDataset = chartData.datasets.find(
    (dataset) => dataset.label === "Entrance Point Count (Customs Area)"
  );

  // Remove the line dataset temporarily (to render other datasets first)
  var datasetsWithoutLine = chartData.datasets.filter(
    (dataset) => dataset.label !== "Entrance Point Count (Customs Area)"
  );
  ...
  var chart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: datasetsWithoutLine.map((dataset) => ({
        ...
        hidden:
          dataset.label === "Total Seats (Waiting Area)" ||
          dataset.label === "Total People (Waiting Area)",
      })),
    },
    options: {
      ...
      plugins: {
        title: {
          ...
          text:
            chartId === "waitingAreaChart"
              ? "Stacked Bar Chart - Waiting Area"
              : "Line Chart - Customs Area",
        },
      },
      responsive: true,
      scales: {
...
function updateHtmlElements(statistics) {
  ...
  if ($.isEmptyObject(statistics)) {
    ...
    updateHtmlElement(
      "#avgCustomOccupancyValue",
      statistics.avg_occupancy_custom + "%"
    );
    updateHtmlElement(
      "#peakCustomOccupancyValue",
      statistics.peak_occupancy_custom + "%"
    );
  }
}
...
function applyDateFilter() {
  ...
  var startDate = $("#dateRangeFilter")
    .data("daterangepicker")
    .startDate.format("YYYY-MM-DD");
  var endDate = $("#dateRangeFilter")
    .data("daterangepicker")
